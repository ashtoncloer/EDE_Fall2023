...
`==` filter and compare to specific value
`!=` filter and check if different that specific value
...
`&` multiple values to check again
`|` if one of the other of multiple values should be true
...
NTL.phys.data.1900thru1999 <- filter(NTL.phys.data, lakename == 'Tuesday Lake' & year4 %in% c(1900:1999))

```
Question: Why don't we filter using row numbers?

> Answer: it would be extremely difficult to count up the number of rows needed to subset in a data set with over 30,000 observations 
...
#dates: 1998, 2002, 1995
#lakes: East Long Lake, Hummingbird lake
#depths: 0 and 0.5
```
### Select
