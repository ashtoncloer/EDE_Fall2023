...
vector1[3] #numeric 5.3 

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) #numeric matrix
matrix1
...
matrix1[4, ] #prints out entire row
matrix1[ , 3] #prints out entire column 
matrix1[c(12, 14)] #counts by row until it hits the 12th and 14th element in the matrix
matrix1[c(12:14)] #counts by row until it hits the 12th, 13th, and 14th element
matrix1[2:4, 1:3] #subsets rows 2-4 and columns 1-3
...
```{r DataExercise-1a}
high_temps <- c(82, 83, 84, 79, 71, 78, 82, 85, 84, 82)
low_temps <- c(56, 58, 60, 62, 62, 64, 64, 64, 62, 60)
```
...
```{r DataExercise-1b}
celsius_function <- function(x){(x-32)*(5/9)}

high_temps_C <- celsius_function(high_temps)
low_temps_C <- celsius_function(low_temps)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
durhamtemps.df <- data.frame("High Temps (F)"=high_temps, "High Temps (C)"=high_temps_C, "Low Temps (F)"=low_temps, "Low Temps (C)"=low_temps_C)
...
```{r DataExercise-1d}
summary(durhamtemps.df)
sd(durhamtemps.df$High.Temps..F.)
```
...
#date_juneteenth <- ()
#date_juneteenth
date_obj_today1 <- dmy(str_juneteenth)
date_obj_today1
```
