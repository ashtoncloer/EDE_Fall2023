...
```{r build plot theme}
#3
mytheme1 <- theme_bw(base_size = 14) +
...
PhosphorusVsPhosphate <- 
  ...
  ggtitle("How Phosphorus Varies by Phosphate Levels") +
  labs(x = "Phosphate", y = "Phosphorus (ug/l)") +
  mytheme1
...
#6
needles_litter <-
  ggplot(subset(NiwotRidge.litter, functionalGroup == "Needles"), aes(x = collectDate, y = dryMass, color = nlcdClass)) +
  geom_point() +
  mytheme1
...
needles_litter2 <-
  ggplot(subset(NiwotRidge.litter, functionalGroup == "Needles"), aes(x = collectDate, y = dryMass, color = nlcdClass)) +
  geom_point() +
  facet_grid(nlcdClass ~ functionalGroup) +
  mytheme1
